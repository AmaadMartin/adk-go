...
func substituteSummaries(events []*session.Event) []*session.Event {
	...
	for i, ev := range events {
		...
		kept = append(kept, keptRange{index: i, rng: unionHolesOverIdenticalRanges(ev.Actions.Compaction, events)})
	}
...
func newCoverIndex(events []*session.Event) coverIndex {
	...
	for j, ev := range events {
		if hasCompaction(ev) {
			idx.at = append(idx.at, j)
			idx.rngs = append(idx.rngs, ev.Actions.Compaction)
		}
	}
	return idx
}
...
// unionHolesOverIdenticalRanges returns rng with the holes of every record
// covering exactly the same interval folded in.
//
...
func unionHolesOverIdenticalRanges(rng *session.EventCompaction, events []*session.Event) *session.EventCompaction {
