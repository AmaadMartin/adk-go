...
graph LR
    ...
    subgraph "ADK Application Workflow"
        ...
        A -.->|"2a. in scope: RunNode + WithUseAsOutput"| D[Agent Node: drafter LLM]
        D -.->|"3a. the drafter's own event is the output"| End((End))
        A --> |"3b. out of scope: assistant returns its own string"| End
    end
    User -- "1. request" --> Start
    End -- "4. reply" --> User
```

Solid arrows are static graph edges. Dotted arrows are the imperative `RunNode`
call and the delegated output it produces. Only one of `3a` and `3b` happens per
turn, decided by `isEmailRequest` before any model call.
