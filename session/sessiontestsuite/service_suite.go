...
func RunServiceTests(t *testing.T, opts SuiteOptions, setup func(t *testing.T) session.Service) {
	...
	t.Run("AppendEvent", func(t *testing.T) {
		...
		t.Run("a_hole_still_names_its_event_after_a_round_trip", func(t *testing.T) {
			...
			// The reference is built from the caller's own event, not from the
			// event read back.
			//
			// Building it from the read-back copy is what this used to do, and
			// it cannot fail: the reference inherits whatever rounding the
			// backend applied, so both sides of the comparison move together
			// and a backend that mangles timestamps still passes. In production
			// the producer holds the in-memory event when it names a hole, and
			// the stored copy is whatever the backend made of it. That mismatch
			// is the thing worth checking.
			//
			// It still cannot fail on a backend that truncates the caller's
			// event in place, session/database among them, because there the
			// two sides are the same object and agree by construction. That is
			// a property of those backends rather than a gap here. It is
			// load-bearing on one that restamps on read instead, which is what
			// session/vertexai does, and that is the backend the question was
			// open on.
			...
			ref := session.EventRef{InvocationID: turn.InvocationID, Timestamp: turn.Timestamp}
